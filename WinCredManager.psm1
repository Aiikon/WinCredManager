...
{
    try { [void][Rhodium.CredManager.CredHelper] } catch {
        # Taken from https://github.com/zetlen/clortho/blob/master/CredMan.ps1
        Add-Type '
        using System;
        using System.Runtime.InteropServices;

        namespace Rhodium.CredManager
        {
...
Function Get-WCMCredential
{
    ...
    (
        ...
        [Parameter()] [ValidateSet('Generic', 'DomainPassword')] [string] $Type,
        [Parameter()] [switch] $AsObject
    )
    End
    {
        Initialize-WinCredManager

        $tempCredentialList = [Rhodium.CredManager.CredHelper+Credential[]]@()
        [void][Rhodium.CredManager.CredHelper]::CredEnum('', [ref]$tempCredentialList)
        
        $credentialList = foreach ($credential in $tempCredentialList | Sort-Object Type, TargetName)
        {
            $isMatch = $credential.TargetName -match '(.+)\:target=(.+)'
            $result = [ordered]@{}
            ...
            try
            {
                $securePassword = ConvertTo-SecureString $credential.CredentialBlob -AsPlainText -Force -ErrorAction Stop
                $result.Credential = New-Object System.Management.Automation.PSCredential $result.UserName, $securePassword
            }
            catch
            {
        ...
        if ($Type) { $credentialList = $credentialList | Where-Object Type -eq $Type }

        if (!$Name) { return $credentialList }

        $credentialList = $credentialList | Where-Object Name -eq $Name

        if ($AsObject) { $credentialList } else { $credentialList.Credential } 
    }
}

Function Add-WCMCredential
{
    Param
    (
        ...
        [Parameter(Mandatory=$true)] [ValidateSet('Generic', 'DomainPassword')] [string] $Type
    )
    End
    {
        Initialize-WinCredManager
        ...
        $wcmCredential = New-Object Rhodium.CredManager.CredHelper+Credential

        $wcmCredential.TargetName = $Name
        $wcmCredential.Flags = [Rhodium.CredManager.CredHelper+CRED_FLAGS]::NONE
        $wcmCredential.Type = [Rhodium.CredManager.CredHelper+CRED_TYPE]::($Script:ShortToFullType[$Type])
        $wcmCredential.UserName = $Username
        $wcmCredential.AttributeCount = 0
        $wcmCredential.Persist = [Rhodium.CredManager.CredHelper+CRED_PERSIST]::ENTERPRISE
        $wcmCredential.CredentialBlobSize = [System.Text.Encoding]::Unicode.GetBytes($Password).Length
        $wcmCredential.CredentialBlob = $Password
        $wcmCredential.Comment = ''

        $writeResult = [Rhodium.CredManager.CredHelper]::CredWrite($wcmCredential)

        if ($writeResult -ne 0)
        {
            throw "Error code '$writeResult' returned when writing credential '$Name'."
        }
    }
}
...
Function Remove-WCMCredential
{
    Param
    (
        [Parameter(ValueFromPipelineByPropertyName=$true, Mandatory=$true, Position=0)] [string] $Name,
        [Parameter(ValueFromPipelineByPropertyName=$true, Mandatory=$true)] [ValidateSet('Generic', 'DomainPassword')] [string] $Type
    )
    Begin
    {
        Initialize-WinCredManager
    ...
    Process
    {
        $result = [Rhodium.CredManager.CredHelper]::CredDelete($Name, [Rhodium.CredManager.CredHelper+CRED_TYPE]::($Script:ShortToFullType[$Type]))
        if ($result -ne 0)
        {
            throw "Error code '$result' returned when deleting credential '$Name'."
        }
    }
}